...
class CharacterPromptBuilder:
    ...
    def build_prompt(
        ...
        # Pass seed for sequential iteration: incrementing seed from 0 iterates through lists in order
        resolved = {
            "character_name": character_name.strip() or None,
            "image_category": choose_for_rating(image_category, data["image_sfw"], data["image_nsfw"], pool_mode, rng, seed),
            "gender": choose(gender, opt["gender"], rng, seed),
            "race": choose(race, opt.get("race") or [], rng, seed),
            "age_group": choose(age_group, opt["age_group"], rng, seed),
            "body_type": choose(body_type, opt["body_type"], rng, seed),
            "hair_color": choose(hair_color, opt["hair_color"], rng, seed),
            "hair_style": choose(hair_style, opt["hair_style"], rng, seed),
            "eye_color": choose(eye_color, opt["eye_color"], rng, seed),
            "facial_expression": choose(facial_expression, opt["facial_expression"], rng, seed),
            "face_angle": choose(face_angle, opt["face_angle"], rng, seed),
            "camera_angle": choose(camera_angle, opt["camera_angle"], rng, seed),
            "pose_style": choose_for_rating(pose_style, data["pose_sfw"], data["pose_nsfw"], pool_mode, rng, seed),
            "makeup_style": choose(makeup_style, opt["makeup_style"], rng, seed),
            "fashion_outfit": choose(fashion_outfit, opt.get("fashion_outfit") or [], rng, seed),
            "fashion_style": choose(fashion_style, opt.get("fashion_style") or [], rng, seed),
            "footwear_style": choose(footwear_style, opt.get("footwear_style") or [], rng, seed),
            "upcycled_fashion": choose(upcycled_fashion, opt.get("upcycled_materials") or [], rng, seed),
            "background_stage_style": choose(background_stage_style, bg["studio_controlled"], rng, seed),
            "background_location_style": choose(background_location_style, bg["public_exotic_real"], rng, seed),
            "background_imaginative_style": choose(background_imaginative_style, bg["imaginative_surreal"], rng, seed),
            "lighting_style": choose(lighting_style, opt.get("lighting_style") or [], rng, seed),
            "camera_lens": choose(camera_lens, opt.get("camera_lens") or [], rng, seed),
            "color_palette": choose(color_palette, opt.get("color_palette") or [], rng, seed),
            "region": choose(region, data["region_options"]["regions"], rng, seed),
            "country": choose(country, data["country_options"]["countries"], rng, seed),
            "culture": choose(culture, data["culture_options"]["cultures"], rng, seed),
        }
